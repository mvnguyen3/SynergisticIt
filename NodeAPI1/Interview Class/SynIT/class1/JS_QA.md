...
- A: `Christopher` and `undefined`
- B: `Christopher` and `ReferenceError`
...
The string `'Christopher'` is a truthy value. What we're actually asking, is "is this truthy value falsy?". This returns `false`.
...
const member = new Person("Christopher", "Hallie");
...
const Christopher = new Person("Christopher", "Hallie");
...
- A: `Person {firstName: "Christopher", lastName: "Hallie"}` and `undefined`
- B: `Person {firstName: "Christopher", lastName: "Hallie"}` and `Person {firstName: "Sarah", lastName: "Smith"}`
- C: `Person {firstName: "Christopher", lastName: "Hallie"}` and `{}`
- D:`Person {firstName: "Christopher", lastName: "Hallie"}` and `ReferenceError`
...
const person = "Christopher";
const age = 21;

getPersonInfo`${person} is ${age} years old`;
```

- A: `"Christopher"` `21` `["", " is ", " years old"]`
- B: `["", " is ", " years old"]` `"Christopher"` `21`
- C: `"Christopher"` `["", " is ", " years old"]` `21`
...
```javascript
String.prototype.giveChristopherPizza = () => {
  return "Just give Christopher pizza already!";
};

const name = "Christopher";

name.giveChristopherPizza();
```

- A: `"Just give Christopher pizza already!"`
...
```javascript
const person = { name: "Christopher" };

function helloSynergisticIT(age) {
  console.log(`${this.name} is ${age}`);
}

helloSynergisticIT.call(person, 21);
helloSynergisticIT.bind(person, 21);
```

- A: `undefined is 21` `Christopher is 21`
- B: `function` `function`
- C: `Christopher is 21` `Christopher is 21`
- D: `Christopher is 21` `function`
...
console.log(typeof helloSynergisticIT());
```
...
The `helloSynergisticIT` function returns the returned value of the immediately invoked function (IIFE). This function returned `0`, which is type `"number"`.
...
- A: `{ name: "Christopher" }, { age: 21 }`
- B: `"name", "age"`
- C: `"Christopher", 21`
- D: `["name", "Christopher"], ["age", 21]`
...
- A: `{ name: "Christopher" }, "1997"`
- B: `{ name: "Sarah" }, "1998"`
- C: `{ name: "Christopher" }, "1998"`
- D: `{ name: "Sarah" }, "1997"`
...
The value of `person` is an object. The argument `member` has a (copied) reference to the _same_ object. When we modify a property of the object `member` has a reference to, the value of `person` will also be modified, since they both have a reference to the same object. `person`'s `name` property is now equal to the value `"Christopher"`
...
```javascript
const user = { name: "Christopher", age: 21 };
const admin = { admin: true, ...user };

console.log(admin);
```

- A: `{ admin: true, user: { name: "Christopher", age: 21 } }`
- B: `{ admin: true, name: "Christopher", age: 21 }`
- C: `{ admin: true, user: ["Christopher", 21] }`
- D: `{ admin: true }`
...
It's possible to combine objects using the spread operator `...`. It lets you create copies of the key/value pairs of one object, and add them to another object. In this case, we create copies of the `user` object, and add them to the `admin` object. The `admin` object now contains the copied key/value pairs, which results in `{ admin: true, name: "Christopher", age: 21 }`.
...
- A: `{ name: "Christopher", age: 21 }`, `["name", "age"]`
- B: `{ name: "Christopher", age: 21 }`, `["name"]`
- C: `{ name: "Christopher"}`, `["name", "age"]`
- D: `{ name: "Christopher"}`, `["age"]`
...
- B: `"{"username": "Christopherhallie"}"`
- C: `"["level", "health"]"`
- D: `"{"username": "Christopherhallie", "level":19, "health":90}"`
...
```javascript
const name = "Christopher Hallie"
console.log(name.padStart(13))
console.log(name.padStart(2))
```

- A: `"Christopher Hallie"`, `"Christopher Hallie"`
- B: `"           Christopher Hallie"`, `"  Christopher Hallie"` (`"[13x whitespace]Christopher Hallie"`, `"[2x whitespace]Christopher Hallie"`)
- C: `" Christopher Hallie"`, `"Christopher Hallie"` (`"[1x whitespace]Christopher Hallie"`, `"Christopher Hallie"`)
- D: `"Christopher Hallie"`, `"Lyd"`, 
...
With the `padStart` method, we can add padding to the beginning of a string. The value passed to this method is the _total_ length of the string together with the padding. The string `"Christopher Hallie"` has a length of `12`. `name.padStart(13)` inserts 1 space at the start of the string, because 12 + 1 is 13.
...
```javascript
const { name: myName } = { name: "Christopher" };
...
When we unpack the property `name` from the object on the right-hand side, we assign its value `"Christopher"` to a variable with the name `myName`.
...
```javascript
function helloSynergisticIT(name) {
  return `Hi there, ${name}`
}

console.log(helloSynergisticIT())
```
...
`function helloSynergisticIT(name = "Christopher") { ... }`

In this case, if we didn't pass a value or if we passed `undefined`, `name` would always be equal to the string `Christopher`
...
- A: `{ name: "Christopher", age: 21 }`
- B: `{ name: "Christopher", age: 21, city: "Amsterdam" }`
- C: `{ name: "Christopher", age: 21, city: undefined }`
- D: `"Amsterdam"`
...
- A: `{ default: function default(), name: "Christopher" }`
- B: `{ default: function default() }`
- C: `{ default: "Hello world", name: "Christopher" }`
...
const member = new Person("Christopher")
...
```javascript
function giveChristopherPizza() {
  return "Here is pizza!"
}

const giveChristopherChocolate = () => "Here's chocolate... now go hit the gym already."

console.log(giveChristopherPizza.prototype)
console.log(giveChristopherChocolate.prototype)
```
...
Regular functions, such as the `giveChristopherPizza` function, have a `prototype` property, which is an object (prototype object) with a `constructor` property. Arrow functions however, such as the `giveChristopherChocolate` function, do not have this `prototype` property. `undefined` gets returned when trying to access the `prototype` property using `giveChristopherChocolate.prototype`. 
...
- A: `name` `Christopher` and `age` `21`
- B: `["name", "Christopher"]` and `["age", 21]` 
...
The first subarray is `[ "name", "Christopher" ]`, with `x` equal to `"name"`, and `y` equal to `"Christopher"`, which get logged.
...
- B: `[1, [2, 3, 4]]` and `{ name: "Christopher", age: 21 }`
- C: `[1, 2, 3, 4]` and `{ name: "Christopher", age: 21 }`
- D: `Error` and `{ name: "Christopher", age: 21 }`
...
- A: `3`, `NaN`, `NaN`
- B: `3`, `7`, `NaN`
- C: `3`, `Christopher2`, `[Object object]2`
- D: `"12"`, `Christopher2`, `[Object object]2`
...
However, the second one is a string `"Christopher"`. `"Christopher"` is a string and `2` is a number: `2` gets coerced into a string. `"Christopher"` and `"2"` get concatenated, which results in the string `"Christopher2"`. 

`{ name: "Christopher" }` is an object. Neither a number nor an object is a string, so it stringifies both. Whenever we stringify a regular object, it becomes `"[Object object]"`. `"[Object object]"` concatenated with `"2"` becomes `"[Object object]2"`.
...
const jsonArray = JSON.stringify({ name: "Christopher" }) // '{"name":"Christopher"}'
JSON.parse(jsonArray) // { name: 'Christopher' }
```
...
If we wouldn't have declared the `name` variable within the `getName` function, the javascript engine would've looked down the _scope chain_. The outer scope has a variable called `name` with the value of `Christopher`. In that case, it would've logged `Christopher`. 
...
- A: `{name: "Sarah", age: 22}`
- B: `{name: "Sarah", age: 23}`
- C: `{name: "Christopher", age: 22}`
- D: `{name: "Christopher", age: 23}`
...
First, we invoke the `changeAge` function and pass the `person` object as its argument. This function increases the value of the `age` property by 1. `person` is now `{ name: "Christopher", age: 22 }`.

Then, we invoke the `changeAgeAndName` function, however we don't pass a parameter. Instead, the value of `x` is equal to a _new_ object: `{ ...person }`. Since it's a new object, it doesn't affect the values of the properties on the `person` object. `person` is still equal to `{ name: "Christopher", age: 22 }`.
